...
def end_ride_api(account, role, ride_id: int):
    ...
    # Zahlungsmittel ist erforderlich
    if not payment_method_id:
        return jsonify({"error": "payment_method_id ist erforderlich."}), 400
    ...
    if not payment_method:
        return jsonify({"error": "Ungültiges oder inaktives Zahlungsmittel."}), 400
