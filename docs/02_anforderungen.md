...
| --- | --- | --- | --- |
| F1 | Benutzer- & Anbieter-Accounts | Registrierung für Fahrer:innen und Anbieter mit eindeutigem Benutzernamen und eindeutiger E-Mail, Passwort-Hashing, Session-Login für Rollen `nutzer` und `verleihanbieter`. | Erfüllt |
...
| F3 | Fahrten-Lifecycle | Fahrer:innen sehen verfügbare Fahrzeuge, starten Fahrten (Statuswechsel auf `in_benutzung`), erfassen Kilometer/Dauer und beenden Fahrten mit Kostenberechnung inkl. Akku-Drain. | Erfüllt |
...
| NF2 | Deployment & Betrieb | Docker Compose startet DB + App; `scripts/init_db.py` setzt Schema, Stored Procedures und Seed-Daten automatisiert auf. | Erfüllt |
...
| NF6 | Dokumentation & Tests | README, Swagger, ERM, Architektur-Doku vorhanden; automatisierte Tests (Pytest) dokumentieren Qualitätsnachweis. | Erfüllt |
